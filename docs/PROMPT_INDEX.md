...
| 102 | PHASE 4 — PREMIÈRE PAGE HISTORIQUE | [ ] *partiel* (recherche Isère + page date + climat annuel + records d’année dont plus de jours ≥ 30 °C, plus de gel et plus de nuits tropicales + comparateur d’années et de communes Isère, HTML initial) |
...
| 125 | PAGE MOBILE | [ ] *partiel* (390 px ; HTML page 56 Ko si `?date=` ; héros sans query = climat LVD 2025 min. 8,2 / max. 19,6 °C · écart min-max 11,4 °C · 144 j de pluie · 110 j ≥ 25 °C · 57 j ≥ 30 °C · 15 j ≥ 35 °C · 0 j ≥ 40 °C · 46 j de gel · 5 nuits tropicales, date vide, partage année ; records d’année = 2003 · 73 j ≥ 30 °C, 2005 · 94 j de gel et 2024 · 7 nuits tropicales ; héros `?date=` = Tmin/Tmax/pluie + station à X km ; accueil = climat officiel LVD ; naissance / comparer = exemples officiels ; Lighthouse lab LCP 2178 ms ; pas CrUX ; pas de bottom sheet GIS) |
...
| 138 | CACHE | [ ] *partiel* (`unstable_cache` 1 h ; yearly **page** `commune-yearly-page-v12` `detailRows: false` ; comparer ville vs ville + `compare-examples-v1` ; accueil `featured-climate-v12` ; naissance `birth-examples-v1` ; `/yearly` complet à la demande ; pas CDN) |
| 139 | PRECOMPUTATION | [ ] |
