...
| 126 | SEO | [ ] *partiel* (titres commune + titres `?date=` / naissance / comparer = mesure officielle ; comparer landing = exemples officiels ; sitemap filtré + OG + hreflang fr + JSON-LD + score v1 ; pas de pages en) |
...
| 138 | CACHE | [ ] *partiel* (`unstable_cache` 1 h ; yearly **page** `detailRows: false` ; comparer ville vs ville + `compare-examples-v1` ; accueil `featured-climate-v2` ; naissance `birth-examples-v1` ; `/yearly` complet à la demande ; pas CDN) |
| 139 | PRECOMPUTATION | [ ] |
