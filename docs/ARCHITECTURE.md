...
SQLite local + `raw/` + cache tuiles IGN (`data/tiles/ign/`, uniquement les tuiles déjà affichées). Accueil, sitemap et **historique d’un jour** (`getPlaceHistory`) : lecture SQLite mise en cache 1 h (`unstable_cache`, `src/lib/sqliteReadCache.ts`) — pas un CDN, pas un appel fournisseur. La page commune envoie la mesure du jour dans le HTML initial (pas d’attente `/api/v1/history` pour le premier écran). Une page vue ne frappe ni data.gouv, ni geo.api, ni Météo-France. IGN : au plus la première fois qu’une tuile manque, 2 requêtes simultanées max. Redis **interdit au lancement** (ADR-0002). `npm run stats:compute` n’est **jamais** déclenché par une page vue.
