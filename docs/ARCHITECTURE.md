...
SQLite local + `raw/` + cache tuiles IGN (`data/tiles/ign/`, uniquement les tuiles déjà affichées). Accueil, sitemap, historique d’un jour, climat annuel (**page** : sans les mois-station, `getCommuneYearlyPageCached`) et **enfance** : lecture SQLite mise en cache 1 h (`unstable_cache`, `src/lib/sqliteReadCache.ts`) — pas un CDN, pas un appel fournisseur. La page commune envoie la mesure du jour, le climat annuel (années / normales / chaleur, pas les ~300 mois) et l’enfance (si naissance) dans le HTML initial. Les mois se chargent près de `#mois` via `GET /yearly` (payload complet). MapLibre et Recharts ne sont demandés que près du viewport. Visuels UI : JPEG des mêmes fichiers, `next/image` AVIF/WebP, héros `decoding=sync` — pas un extract IGN massif. Une page vue ne frappe ni data.gouv, ni geo.api, ni Météo-France. IGN : au plus la première fois qu’une tuile manque, 2 requêtes simultanées max. Redis **interdit au lancement** (ADR-0002). `npm run stats:compute` n’est **jamais** déclenché par une page vue.
