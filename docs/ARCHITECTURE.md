...
| ERA5 point Grenoble (1 jour, 2t + d2m + TP + vent + MSL + SP + neige SWE + SSRD + rafale) + quotidien 2t France 3 jours (JSON) | R9 remainder : archive quotidienne 1940–2026, pas mondial |
...
SQLite local + `raw/` + cache tuiles IGN (`data/tiles/ign/`, uniquement les tuiles déjà affichées). Accueil, sitemap, historique d’un jour, climat annuel (**page** : sans mois / saisons / chaleur, `getCommuneYearlyPageCached`, `detailRows: false`), **enfance** et **ville vs ville** : lecture SQLite mise en cache 1 h (`unstable_cache`, `src/lib/sqliteReadCache.ts`) — pas un CDN, pas un appel fournisseur. La page commune envoie la mesure du jour, le climat annuel (années / normales / records, pas les mois ni les saisons ni le scan quotidien de chaleur) et l’enfance (si naissance) dans le HTML initial. Les détails se chargent près de `#mois` via `GET /yearly` (payload complet). Un tableau chaleur vide dans le HTML lite n’est pas « aucun épisode ». MapLibre et Recharts ne sont demandés que près du viewport. Visuels UI : JPEG des mêmes fichiers, `next/image` AVIF/WebP, héros `decoding=sync` quality 60 — pas un extract IGN massif. Une page vue ne frappe ni data.gouv, ni geo.api, ni Météo-France. IGN : au plus la première fois qu’une tuile manque, 2 requêtes simultanées max. Redis **interdit au lancement** (ADR-0002). `npm run stats:compute` n’est **jamais** déclenché par une page vue.
