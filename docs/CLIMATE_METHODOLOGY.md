...
- Réanalyse : maille la plus proche **45,25°N, 5,75°E**, 24 heures UTC. `2m_temperature` min/max → 3,4 / 14,2 °C. Point de rosée `2m_dewpoint_temperature` min/max → **2,0 / 6,9 °C**. Pluie `total_precipitation` : somme des 24 pas horaires ARCO (**0,00032344 m** → **0,3 mm**). Vent 10 m : moyenne des hypot(u,v) **8,8 km/h**, d’où il vient **169°**. MSL moyenne 24 h **1005 hPa**. Ce n’est **pas** le Tmin/Tmax/Td d’abri, ni un pluviomètre, ni un anémomètre, ni la pression au sol.
- Écart = estimation − mesure (2t seulement dans le bandeau) : Tmin **−3,2 °C**, Tmax **−7,4 °C**. Pluie : **+0,2 mm** (0,3 − 0,1), seulement dans « En savoir plus ». Pas d’écart vent/MSL : le quotidien Météo-France importé n’a pas ces variables. Affiché, **pas fusionné**. Rosée, pluie, vent et MSL ERA5 n’entrent pas dans le JSON-LD.
- `method_version` point : `era5-point-nearest-hourly-2t-d2m-tp-uv10-msl-v1`. Quotidien France un jour : `era5-france-daily-2t-minmax-v1` (JSON, pas SQL). Version jeu : ERA5 finale (1983).
