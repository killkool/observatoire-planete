...
**Date de revue :** 2026-09-07 (heatmap « ce jour » en liens date)  
...
| R12 mobile | **Partiel** | 390 px ; HTML initial 56 Ko sans mois/saisons/chaleur ; heatmap « ce jour » = liens `?date=` ; Lighthouse lab LCP 2535 ms ; pas CDN, pas CrUX |
...
- Lighthouse **13.4.1** mobile, `next start` :3002, Grenoble `?date=1983-05-12`, 2026-09-06T22:26Z : performance **0,97** ; FCP **0,9 s** ; LCP **2,5 s** (**2535 ms**, score 0,89) ; TBT **22 ms** ; CLS **0**. Tentative 2026-09-07T00:26Z : héros WebP 828 px hors `/_next/image` → LCP **2779 ms** (perf 0,96) ; revert, pas un pass. Lab localhost, pas CrUX. Case Core Web Vitals **non** cochée.
...
Tests : `npm run test:science`. Pages : `/meteo/auvergne-rhone-alpes/isere/grenoble?date=1983-05-12` · `?date=1983-05-11` · `?date=1983-05-13` · `?date=1982-05-12` (CORENC 5,1 / 26,1 °C, ERA5 2,8 / 18,6 °C) · `?date=1986-05-12` (CORENC 11,6 / 32,1 °C, ERA5 8,6 / 22,2 °C, ΔTmax −9,9 °C) · heatmap 1986 → lien 1982 (URL + mesures) · `?date=1900-01-01` · `/` · `/naissance` · `/comparer?a=38185&b=38563`.
