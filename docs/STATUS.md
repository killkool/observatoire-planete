...
**Date de revue :** 2026-09-07 (année climatique canonique = 57 jours officiels ≥ 30 °C)  
...
| R11 SEO | **Fait (Isère)** | titres, héros et OG commune sans query = dernière année climatique officielle (min. 8,2 / max. 19,6 °C · **57 j ≥ 30 °C**) ; landings OG `/og/accueil` (exemple Grenoble 2025, pas Crolles) `/og/naissance` (1983 CORENC) `/og/comparer` (Voiron, pas `?a=&b=`) ; JSON-LD jour seulement si `?date=` ; titres date/naissance/comparer officiels + sitemap + hreflang + `seo-content-v1` ; pas de pages en |
| R12 mobile | **Partiel** | 390 px ; HTML initial 56 Ko (`?date=`) ; héros sans query = climat 2025 (min. 8,2 °C · **57 j ≥ 30 °C**) sans jour par défaut, date vide, partage année ; héros `?date=` = Tmin/Tmax/pluie + station ; comparer SSR + exemples ; accueil = climat officiel LVD 2025 ; naissance = exemples officiels ; Lighthouse lab LCP 2178 ms ; pas CDN, pas CrUX |
...
- Page commune 390×844 : nav horizontale défilable, date en colonne. Sans query : héros = **année climatique 2025 · min. 8,2 °C · max. 19,6 °C · 901,1 mm · 57 j ≥ 30 °C** + LVD 10,2 km ; sélecteur de date **vide** ; **pas** de heatmap du 4 septembre ni de 12,8 / 35,4 °C. Avec `?date=` : héros = **Tmin/Tmax/pluie officiels** + station à X km (CORENC 4,7 km), pas d’ERA5. Naissance : récit **0,1 mm** + CORENC. Carte IGN au scroll si un jour est demandé. Cibles ≥ 44 px. Pas de bottom sheet.
- Accueil / naissance / comparer 390×844 : recherche et CTA pleine largeur, 1 colonne, overflow-x absent, cibles ≥ 44 px. Cartes d’accueil : **2025 · min. 8,2 °C · max. 19,6 °C · 901,1 mm · 57 j ≥ 30 °C**, station **GRENOBLE - LVD** (Grenoble 10,2 km, Crolles 8,1 km, La Pierre 11,6 km ; même poste, pas d’écart inventé, pas le record 2022 **71 j**). `/naissance` : exemples Grenoble **1983-05-12** (6,6 / 21,6 °C, 0,1 mm, CORENC 4,7 km), **1986-05-12** (0,0 mm ≠ ERA5 2,1 mm), **1900-01-01** (aucune mesure). `/comparer` sans query : Grenoble vs Voiron **18,5 → 18,2 °C** (LVD / COUBLEVIE) et vs Crolles **même station** dans le premier HTML. Canonical `/comparer`.
...
- Cache Next `unstable_cache` 1 h : accueil **climat officiel** (`featured-climate-v3`) + **exemples naissance** (`birth-examples-v1`) + **exemples comparer** (`compare-examples-v1`) + sitemap **517** URL + historique d’un jour + climat annuel (page : **sans** mois / saisons / chaleur, `commune-yearly-page-v2`, `detailRows: false`) + enfance + **ville vs ville**. HTML Grenoble 1983-05-12 : **56 Ko** (avant 78 Ko), jour CORENC 6,6 / 21,6 °C, climat LVD 2025 · 901,1 mm · 57 j ≥ 30 °C. Les détails se chargent à l’approche de `#mois` (`GET /yearly` complet). Un payload chaleur vide n’affiche pas « aucun épisode » : texte de chargement jusqu’à l’API. Date 1900-01-01 : aucune mesure inventée.
