...
**Date de revue :** 2026-09-07 (année climatique canonique = min./max. officielles + partage, pas le jour par défaut)  
...
Le parcours **Isère → Météo-France → Grenoble → 1983-05-12 → ERA5 point (2t + rosée + pluie + vent 10 m + MSL + SP + neige SWE + SSRD + rafale) → comparaison sans fusion → provenance → confiance** est livré. Même point **5 jours**. Quotidien 2t bbox France : **3 jours**. Héros = Tmin/Tmax/pluie officiels + station à X km. Naissance = récit + titres + exemples SSR. Comparer = HTML initial + titres + **exemples SSR** (Voiron : 18,5 → 18,2 °C ; Crolles : même station). Accueil = cartes climat LVD 2025 (min. **8,2 °C** · max. 19,6 °C · 901,1 mm). Pages commune sans query = titre, héros **et OG** **2025 · min. 8,2 °C · max. 19,6 °C** (LVD, pas le record 2022, pas le jour par défaut) ; date vide ; partage de l’année. LCP lab **2178 ms**.
...
| R11 SEO | **Fait (Isère)** | titres, héros et OG commune sans query = dernière année climatique officielle (Grenoble 2025 · min. 8,2 °C · max. 19,6 °C · 901,1 mm, LVD 10,2 km, `/og/climat/grenoble`) ; pas le jour par défaut ; JSON-LD jour seulement si `?date=` ; titres date/naissance/comparer officiels + sitemap + hreflang + `seo-content-v1` ; pas de pages en |
| R12 mobile | **Partiel** | 390 px ; HTML initial 56 Ko (`?date=`) ; héros sans query = climat 2025 (min. 8,2 °C) sans jour par défaut, date vide, partage année ; héros `?date=` = Tmin/Tmax/pluie + station ; comparer SSR + exemples ; accueil = climat officiel LVD 2025 ; naissance = exemples officiels ; Lighthouse lab LCP 2178 ms ; pas CDN, pas CrUX |
