...
**Date de revue :** 2026-09-07 (comparer : exemples officiels sur l’accueil)  
...
Le parcours **Isère → Météo-France → Grenoble → 1983-05-12 → ERA5 point (2t + rosée + pluie + vent 10 m + MSL + SP + neige SWE + SSRD + rafale) → comparaison sans fusion → provenance → confiance** est livré. Même point **5 jours**. Quotidien 2t bbox France : **3 jours**. Héros = Tmin/Tmax/pluie officiels + station à X km. Naissance = récit + titres + exemples SSR. Comparer = HTML initial + titres + **exemples SSR** (Voiron : 18,5 → 18,2 °C ; Crolles : même station). Accueil = cartes climat LVD 2025. LCP lab **2178 ms**.
...
| R11 SEO | **Fait (Isère)** | titres commune + titres date/naissance/comparer officiels + sitemap filtré + OG + hreflang fr/x-default + JSON-LD + `seo-content-v1` ; pas de pages en |
