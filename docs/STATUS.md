...
**Date de revue :** 2026-09-07 (ERA5 SP / neige SWE / SSRD / rafale Grenoble)  
...
Le parcours **Isère → Météo-France → Grenoble → 1983-05-12 → ERA5 point (2t + rosée + pluie + vent 10 m + MSL + SP + neige SWE + SSRD + rafale) → comparaison sans fusion → provenance → confiance** est livré. SP **895 hPa** (maille 985,5 m). Neige **27,3 mm d’eau**. SSRD **20,8 MJ/m²**. Rafale **69,4 km/h**. LCP lab **2535 ms** — pas un pass Core Web Vitals.

## Prochaine action

1. **Science / R9 remainder :** quotidien France encore **un jour** 2t (pas 1940–2026). Pas d’archive, pas d’import SQL de la grille.  
...
| R9 ERA5 | **Partiel** | point Grenoble 2t + d2m + TP + 10u/v + MSL + SP + neige SWE + SSRD + rafale 1983-05-12 ; quotidien 2t bbox France un jour (JSON) ; pas 1940–2026 |
...
| PoC 1 ERA5 | **Fait** | `point_extractions` = 14 |
...
Tests : `npm run test:science`. Pages : `/meteo/auvergne-rhone-alpes/isere/grenoble?date=1983-05-12` (CORENC 6,6 / 21,6 °C, ERA5 3,4 / 14,2 °C, rosée 2,0 / 6,9 °C, pluie 0,3 mm, vent 8,8 km/h / 169°, MSL 1005 hPa, SP 895 hPa, neige 27,3 mm d’eau, SSRD 20,8 MJ/m², rafale 69,4 km/h dans En savoir plus) · `?date=1900-01-01` · `/` · `/naissance` · `/comparer?a=38185&b=38563`.
...
| ERA5 | 14 points (Grenoble 1983-05-12, 2t + d2m + pluie + vent + MSL + SP + neige SWE + SSRD + rafale) |
