...
**Date de revue :** 2026-09-07 (naissance : exemples officiels dans le HTML)  
...
Le parcours **Isère → Météo-France → Grenoble → 1983-05-12 → ERA5 point (2t + rosée + pluie + vent 10 m + MSL + SP + neige SWE + SSRD + rafale) → comparaison sans fusion → provenance → confiance** est livré. Même point **5 jours**. Quotidien 2t bbox France : **3 jours**. Héros = Tmin/Tmax/pluie officiels + station à X km. Naissance = récit + titres + **exemples SSR** (1983 : 0,1 mm CORENC ; 1986 : 0,0 mm ≠ ERA5 2,1 mm ; 1900 sans invention). Comparer = HTML initial + titres = stations officielles (même poste = pas d’écart). Accueil = cartes climat LVD 2025. LCP lab **2178 ms**.
...
| R8 comparateur | **Partiel** | année vs année + saison vs **même** saison + ville vs ville Isère (HTML + titres) ; pas hiver vs été, pas France entière |
...
| R12 mobile | **Partiel** | 390 px ; HTML initial 56 Ko ; héros = Tmin/Tmax/pluie + station ; comparer SSR ; accueil = climat officiel LVD 2025 ; naissance = exemples officiels ; Lighthouse lab LCP 2178 ms ; pas CDN, pas CrUX |
