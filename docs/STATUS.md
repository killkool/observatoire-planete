...
**Date de revue :** 2026-09-07 (page commune canonique = climat, pas le jour par défaut)  
...
Le parcours **Isère → Météo-France → Grenoble → 1983-05-12 → ERA5 point (2t + rosée + pluie + vent 10 m + MSL + SP + neige SWE + SSRD + rafale) → comparaison sans fusion → provenance → confiance** est livré. Même point **5 jours**. Quotidien 2t bbox France : **3 jours**. Héros = Tmin/Tmax/pluie officiels + station à X km. Naissance = récit + titres + exemples SSR. Comparer = HTML initial + titres + **exemples SSR** (Voiron : 18,5 → 18,2 °C ; Crolles : même station). Accueil = cartes climat LVD 2025. Pages commune sans query = titre, héros **et OG** **2025 · max. 19,6 °C** (LVD, pas le record 2022, pas le jour par défaut). LCP lab **2178 ms**.
...
| R11 SEO | **Fait (Isère)** | titres, héros et OG commune sans query = dernière année climatique officielle (Grenoble 2025 · 19,6 °C · 901,1 mm, LVD 10,2 km, `/og/climat/grenoble`) ; pas le jour par défaut ; JSON-LD jour seulement si `?date=` ; titres date/naissance/comparer officiels + sitemap + hreflang + `seo-content-v1` ; pas de pages en |
| R12 mobile | **Partiel** | 390 px ; HTML initial 56 Ko (`?date=`) ; héros sans query = climat 2025 sans jour par défaut ; héros `?date=` = Tmin/Tmax/pluie + station ; comparer SSR + exemples ; accueil = climat officiel LVD 2025 ; naissance = exemples officiels ; Lighthouse lab LCP 2178 ms ; pas CDN, pas CrUX |
...
- Carte de partage : `/og/grenoble/1983-05-12` → PNG. Date sans mesure : `/og/grenoble/1900-01-01` → PNG « aucune mesure officielle », pas de 0 inventé. URL canonique : `/og/climat/grenoble` → PNG 2025 · max. 19,6 °C · 901,1 mm, LVD 10,2 km, pas le record 2022, pas le 4 septembre. Pas d’ERA5 sur la carte.
- SEO page commune : canonical + hreflang `fr` / `x-default` vers `/meteo/auvergne-rhone-alpes/isere/grenoble`. JSON-LD `City` INSEE **38185**, geo 45,1885 / 5,7245. Pour `?date=1983-05-12` : titre **6.6 / 21.6 °C**, `WeatherObservation` CORENC 6,6 / 21,6 °C, 0,1 mm. `?histoire=naissance` : titre « Naissance à Grenoble ». `?date=1986-05-12` : pluie **0.0 mm**, pas 2,1 mm ERA5. `?date=1900-01-01` : « aucune mesure officielle », pas de 0 inventé. URL sans query : titre, héros **et OG** **2025 · max. 19,6 °C**, pluie **901,1 mm**, LVD **10,2 km** — pas le record 2022 (20,2 °C), pas le jour par défaut (12,8 / 35,4 °C, heatmap 04/09), pas d’ERA5. JSON-LD sans `WeatherObservation`. OG `/og/climat/grenoble`. Pas d’ERA5 dans le graphe ni dans les balises.
...
- Page commune 390×844 : nav horizontale défilable, date en colonne. Sans query : héros = **année climatique 2025 · max. 19,6 °C · 901,1 mm** + LVD 10,2 km ; **pas** de heatmap du 4 septembre ni de 12,8 / 35,4 °C. Avec `?date=` : héros = **Tmin/Tmax/pluie officiels** + station à X km (CORENC 4,7 km), pas d’ERA5. Naissance : récit **0,1 mm** + CORENC. Carte IGN au scroll si un jour est demandé. Cibles ≥ 44 px. Pas de bottom sheet.
- Accueil / naissance / comparer 390×844 : recherche et CTA pleine largeur, 1 colonne, overflow-x absent, cibles ≥ 44 px. Cartes d’accueil : **2025 · max. 19,6 °C · 901,1 mm**, station **GRENOBLE - LVD** (Grenoble 10,2 km, Crolles 8,1 km, La Pierre 11,6 km ; même poste, pas d’écart inventé, pas le record 2022). `/naissance` : exemples Grenoble **1983-05-12** (6,6 / 21,6 °C, 0,1 mm, CORENC 4,7 km), **1986-05-12** (0,0 mm ≠ ERA5 2,1 mm), **1900-01-01** (aucune mesure). `/comparer` sans query : Grenoble vs Voiron **18,5 → 18,2 °C** (LVD / COUBLEVIE) et vs Crolles **même station** dans le premier HTML. Canonical `/comparer`.
...
- Cache Next `unstable_cache` 1 h : accueil **climat officiel** (`featured-climate-v2`) + **exemples naissance** (`birth-examples-v1`) + **exemples comparer** (`compare-examples-v1`) + sitemap **517** URL + historique d’un jour + climat annuel (page : **sans** mois / saisons / chaleur, `commune-yearly-page-v2`, `detailRows: false`) + enfance + **ville vs ville**. HTML Grenoble 1983-05-12 : **56 Ko** (avant 78 Ko), jour CORENC 6,6 / 21,6 °C, climat LVD 2025 · 901,1 mm. Les détails se chargent à l’approche de `#mois` (`GET /yearly` complet). Un payload chaleur vide n’affiche pas « aucun épisode » : texte de chargement jusqu’à l’API. Date 1900-01-01 : aucune mesure inventée.
...
Tests : `npm run test:science`. Pages : `/meteo/auvergne-rhone-alpes/isere/grenoble` (titre, héros et OG 2025 · max. 19.6 °C, 901.1 mm, LVD 10.2 km, pas 20.2 °C / 2022, pas 12.8 / 35.4 °C ni heatmap 04/09, pas de WeatherObservation JSON-LD, og:image `/og/climat/grenoble`) · `/meteo/auvergne-rhone-alpes/isere/grenoble?date=1983-05-12` (héros 6.6 °C / 21.6 °C / 0.1 mm, CORENC 4.7 km, pas d’ERA5) · `?date=1986-05-12` (pluie 0.0 mm, pas 2.1 mm ERA5) · `?date=1900-01-01` (aucune mesure) · `?date=1983-05-12&histoire=naissance` (récit 0.1 mm + CORENC ; titre Naissance) · heatmap 1986 → 1982 · `/` (cartes 2025 · max. 19.6 °C · 901.1 mm, LVD 10.2 km, pas ERA5, pas 20.2 °C / 2022) · `/naissance` (exemples 1983 0.1 mm CORENC, 1986 0.0 mm, 1900 sans invention, HTML initial) · `/comparer` (exemples Voiron 18.5 → 18.2 °C, Crolles même station, HTML initial, canonical /comparer) · `/comparer?a=38185&b=38563` (18.5 → 18.2 °C, COUBLEVIE, HTML initial) · `/comparer?a=38185&b=38140` (même station LVD).
