...
**Date de revue :** 2026-09-07 (accueil : climat officiel sur les cartes)  
...
Le parcours **Isère → Météo-France → Grenoble → 1983-05-12 → ERA5 point (2t + rosée + pluie + vent 10 m + MSL + SP + neige SWE + SSRD + rafale) → comparaison sans fusion → provenance → confiance** est livré. Même point **5 jours**. Quotidien 2t bbox France : **3 jours**. Héros = Tmin/Tmax/pluie officiels + station à X km. Naissance = même pluie/station dans le récit ; titres `?date=` / `histoire=naissance` = mesure officielle. Comparer = HTML initial + titres = stations officielles (même poste = pas d’écart). Accueil = cartes Grenoble / Crolles / La Pierre = dernière année climatique LVD (**2025 · max. 19,6 °C · 901,1 mm**, 10,2 km), pas le record 2022. LCP lab **2178 ms**.
...
| R6 naissance | **Fait** | `/naissance` + enfance + OG PNG + ce jour (moyenne ≥ 5 ans) ; récit + titres = mesure officielle ; pas de SDK social |
...
| R12 mobile | **Partiel** | 390 px ; HTML initial 56 Ko ; héros = Tmin/Tmax/pluie + station ; comparer SSR ; accueil = climat officiel LVD 2025 ; Lighthouse lab LCP 2178 ms ; pas CDN, pas CrUX |
...
- Accueil / naissance / comparer 390×844 : recherche et CTA pleine largeur, 1 colonne, overflow-x absent, cibles ≥ 44 px. Cartes d’accueil : **2025 · max. 19,6 °C · 901,1 mm**, station **GRENOBLE - LVD** (Grenoble 10,2 km, Crolles 8,1 km, La Pierre 11,6 km ; même poste, pas d’écart inventé, pas le record 2022). Comparer Grenoble vs Voiron : LVD vs COUBLEVIE, 21 années, écarts inchangés.
