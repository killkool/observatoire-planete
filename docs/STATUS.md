...
**Date de revue :** 2026-09-06 (R12 accueil mobile + cache 1 h)  
...
Le parcours **Isère → Météo-France → Grenoble → 1983-05-12 → ERA5 point → comparaison sans fusion → provenance → confiance** est livré. Accueil, naissance, comparer et page commune tiennent à 390 px. Recharts n’est plus dans le JS initial de la page commune. Accueil et sitemap : cache Next 1 h (SQLite, pas un CDN). Pas de score Lighthouse.

## Prochaine action

1. **Produit :** mesurer les Core Web Vitals (Lighthouse / terrain) ; CDN ou vues matérialisées si besoin démontré.  
...
| R12 mobile | **Partiel** | commune + accueil + naissance + comparer 390 px ; Recharts à la demande ; cache 1 h accueil/sitemap ; pas CWV mesuré, pas CDN |
...
- Cache Next `unstable_cache` 1 h : accueil (3 communes) + sitemap **517** URL (5 + 512). Pas un CDN, pas un appel fournisseur. Le dashboard `/dashboard` charge encore Recharts en tête (prototype).
...
Tests : `npm run test:science`. Pages : `/` · `/naissance` · `/comparer?a=38185&b=38563` · `/meteo/auvergne-rhone-alpes/isere/grenoble?date=1983-05-12` · `/sitemap.xml` (5 + 512) · `#ce-jour` · `#comparaison-sources` · `#mois` · `#normale-mensuelle` · `#saisons` · `#chaleur` · `/og/grenoble/1983-05-12`.
