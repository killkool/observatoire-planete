...
**Date de revue :** 2026-09-07 (héros commune = dernière année climatique officielle)  
...
Le parcours **Isère → Météo-France → Grenoble → 1983-05-12 → ERA5 point (2t + rosée + pluie + vent 10 m + MSL + SP + neige SWE + SSRD + rafale) → comparaison sans fusion → provenance → confiance** est livré. Même point **5 jours**. Quotidien 2t bbox France : **3 jours**. Héros = Tmin/Tmax/pluie officiels + station à X km. Naissance = récit + titres + exemples SSR. Comparer = HTML initial + titres + **exemples SSR** (Voiron : 18,5 → 18,2 °C ; Crolles : même station). Accueil = cartes climat LVD 2025. Pages commune sans query = titre **et héros** **2025 · max. 19,6 °C** (LVD, pas le record 2022, pas le jour par défaut). LCP lab **2178 ms**.
...
| R11 SEO | **Fait (Isère)** | titres **et héros** commune sans query = dernière année climatique officielle (Grenoble 2025 · 19,6 °C · 901,1 mm, LVD 10,2 km) ; JSON-LD jour seulement si `?date=` ; titres date/naissance/comparer officiels + sitemap + OG + hreflang + `seo-content-v1` ; pas de pages en |
| R12 mobile | **Partiel** | 390 px ; HTML initial 56 Ko ; héros sans query = climat 2025 ; héros `?date=` = Tmin/Tmax/pluie + station ; comparer SSR + exemples ; accueil = climat officiel LVD 2025 ; naissance = exemples officiels ; Lighthouse lab LCP 2178 ms ; pas CDN, pas CrUX |
...
- SEO page commune : canonical + hreflang `fr` / `x-default` vers `/meteo/auvergne-rhone-alpes/isere/grenoble`. JSON-LD `City` INSEE **38185**, geo 45,1885 / 5,7245. Pour `?date=1983-05-12` : titre **6.6 / 21.6 °C**, `WeatherObservation` CORENC 6,6 / 21,6 °C, 0,1 mm. `?histoire=naissance` : titre « Naissance à Grenoble ». `?date=1986-05-12` : pluie **0.0 mm**, pas 2,1 mm ERA5. `?date=1900-01-01` : « aucune mesure officielle », pas de 0 inventé. URL sans query : titre **et héros** **2025 · max. 19,6 °C**, pluie **901,1 mm**, LVD **10,2 km** — pas le record 2022 (20,2 °C), pas le jour par défaut, pas d’ERA5. JSON-LD sans `WeatherObservation` sur cette URL. Pas d’ERA5 dans le graphe ni dans les balises.
...
- Page commune 390×844 : nav horizontale défilable, date + partage en colonne. Sans query : héros = **année climatique 2025 · max. 19,6 °C · 901,1 mm** + LVD 10,2 km. Avec `?date=` : héros = **Tmin/Tmax/pluie officiels** + station à X km (CORENC 4,7 km), pas d’ERA5. Naissance : récit **0,1 mm** + CORENC. Carte IGN au scroll. Cibles ≥ 44 px. Pas de bottom sheet.
