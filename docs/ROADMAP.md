...
Dernière mise à jour : 2026-09-07 (année climatique canonique = jours officiels ≥ 25 °C). Tableau : [STATUS.md](./STATUS.md).
...
38. [x] Phase R1/R4/R12 — accueil : cartes Grenoble / Crolles / La Pierre = dernière année climatique officielle (LVD 2025 · max. 19,6 °C · 901,1 mm · **110 j ≥ 25 °C** · **57 j ≥ 30 °C** · **15 j ≥ 35 °C** · **46 j de gel** · **5 nuits tropicales**, 10,2 km, pas le record 2022) ; même poste = mêmes chiffres, pas d’écart inventé ; cache `featured-climate-v9`
...
51. Ne **pas** extraire ERA5 mondial. Ne **pas** activer océan / NOAA / API commerciale. Ne **pas** feindre Vercel/Supabase. Ne **pas** extraire massivement les tuiles IGN.
...
**État :** titres et descriptions issus du nom officiel ; page commune **sans query** = dernière année climatique officielle (Grenoble 2025 · min. 8,2 °C · max. 19,6 °C · 901,1 mm · **110 j ≥ 25 °C** · **57 j ≥ 30 °C** · **15 j ≥ 35 °C** · **46 j de gel** · **5 nuits tropicales**, LVD 10,2 km, pas le record 2022 **71 j** / **18 j ≥ 35 °C**, pas 2003 **22 j ≥ 35 °C**, pas 2018 **135 j ≥ 25 °C**, pas 2022 **126 j ≥ 25 °C**, pas le gel 2005 **94 j**, pas les nuits 2024 **7**) dans le titre (max.), le héros (min./max./pluie/jours ≥ 25 °C/≥ 30 °C/≥ 35 °C/gel/nuits tropicales), la carte OG `/og/climat/grenoble` et le texte de partage (URL sans `?date=`) ; sélecteur de date vide ; pas le jour par défaut dans le HTML ni le JSON-LD ; `?date=` et `histoire=naissance` = mesure officielle (pas ERA5) ; `/naissance` = exemples officiels 1983/1986/1900 dans le HTML + OG `/og/naissance` (1983 CORENC, pas 1900, pas ERA5) ; `/comparer` = exemples officiels Voiron / Crolles dans le HTML, canonical `/comparer`, OG landing `/og/comparer` (Voiron, pas collé sur `?a=&b=`) ; accueil OG `/og/accueil` = exemple Grenoble 2025 (pas Crolles, pas une moyenne France). Sitemap = communes dont le score repose sur l’identité INSEE **et** des années climatiques ou des mesures OBSERVED. Hreflang français seulement. JSON-LD : lieu ; `WeatherObservation` seulement si `?date=` / naissance et mesure OBSERVED ; pas d’ERA5. Pas de millions de coquilles.
...
**État :** premier HTML Grenoble **56 Ko** (`?date=`). Heatmap « ce jour » : liens `?date=`. Héros : sans query = année climatique LVD 2025 (min. **8,2 °C** · max. 19,6 °C · 901,1 mm · **110 j ≥ 25 °C** · **57 j ≥ 30 °C** · **15 j ≥ 35 °C** · **46 j de gel** · **5 nuits tropicales**), **sans** le jour par défaut, sélecteur de date vide, partage de l’année ; avec `?date=` = Tmin/Tmax/pluie officiels + station à X km, pas d’ERA5 ni de JPEG IGN ; 1900 = « aucune mesure officielle » ; 1986 pluie héros **0,0 mm** ≠ ERA5 2,1 mm. Accueil : climat officiel LVD 2025 (min. 8,2 °C · 110 j ≥ 25 °C · 57 j ≥ 30 °C · 15 j ≥ 35 °C · 46 j de gel · 5 nuits tropicales) dans le premier HTML. Naissance : exemples officiels dans le premier HTML. Comparer : exemples Voiron / Crolles dans le premier HTML. Lighthouse lab 2026-09-07T00:45Z : perf 99, LCP **2178 ms**, CLS 0 — pas CrUX. Pas de bottom sheet GIS. Pas de CDN.
