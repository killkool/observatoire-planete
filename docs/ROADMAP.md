...
Dernière mise à jour : 2026-09-07 (page commune canonique = climat, pas le jour par défaut). Tableau : [STATUS.md](./STATUS.md).
...
44. Ne **pas** extraire ERA5 mondial. Ne **pas** activer océan / NOAA / API commerciale. Ne **pas** feindre Vercel/Supabase. Ne **pas** extraire massivement les tuiles IGN.
...
**État :** titres et descriptions issus du nom officiel ; page commune **sans query** = dernière année climatique officielle (Grenoble 2025 · 19,6 °C · 901,1 mm, LVD 10,2 km, pas le record 2022) dans le titre, le héros et la carte OG `/og/climat/grenoble` ; pas le jour par défaut dans le HTML ni le JSON-LD ; `?date=` et `histoire=naissance` = mesure officielle (pas ERA5) ; `/naissance` = exemples officiels 1983/1986/1900 dans le HTML ; `/comparer` = exemples officiels Voiron / Crolles dans le HTML, canonical `/comparer`. Sitemap = communes dont le score repose sur l’identité INSEE **et** des années climatiques ou des mesures OBSERVED. Hreflang français seulement. JSON-LD : lieu ; `WeatherObservation` seulement si `?date=` / naissance et mesure OBSERVED ; pas d’ERA5. Pas de millions de coquilles.
