...
Dernière mise à jour : 2026-09-06 (mesure du jour dans le HTML initial). Tableau : [STATUS.md](./STATUS.md).
...
21. Ne **pas** extraire ERA5 mondial. Ne **pas** activer océan / NOAA / API commerciale. Ne **pas** feindre Vercel/Supabase. Ne **pas** extraire massivement les tuiles IGN.
...
**État :** pages grand public utilisables au téléphone. Cache Next 1 h sur accueil, sitemap et historique d’un jour (`src/lib/sqliteReadCache.ts`) — pas un CDN, pas de vues matérialisées. Premier HTML Grenoble : CORENC 6,6 / 21,6 °C, sans fallback « Chargement des observations… ». Pas de bottom sheet ni sélecteur de couches GIS (prompt §125, reste ouvert). Pas de mesure Core Web Vitals (Lighthouse / CrUX).
