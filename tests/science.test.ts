...
assert.equal(formatDaysGe25(1), "1 jour ≥ 25 °C");
assert.equal(formatDaysGe25(110), "110 jours ≥ 25 °C");
assert.equal(formatDaysGe25(null), "non disponible");
assert.equal(formatDaysGe30(0), "0 jour ≥ 30 °C");
assert.equal(formatDaysGe30(1), "1 jour ≥ 30 °C");
assert.equal(formatDaysGe30(57), "57 jours ≥ 30 °C");
assert.equal(formatDaysGe30(null), "non disponible");
assert.equal(formatDaysGe35(0), "0 jour ≥ 35 °C");
assert.equal(formatDaysGe35(1), "1 jour ≥ 35 °C");
assert.equal(formatDaysGe35(15), "15 jours ≥ 35 °C");
assert.equal(formatDaysGe35(null), "non disponible");
assert.equal(formatDaysGe40(0), "0 jour ≥ 40 °C");
assert.equal(formatDaysGe40(1), "1 jour ≥ 40 °C");
assert.equal(formatDaysGe40(8), "8 jours ≥ 40 °C");
assert.equal(formatDaysGe40(null), "non disponible");
assert.equal(formatDaysRain(0), "0 jour de pluie");
...
assert.equal(formatDaysFrost(null), "non disponible");
assert.equal(formatTropicalNights(0), "0 nuit tropicale");
assert.equal(formatTropicalNights(1), "1 nuit tropicale");
assert.equal(formatTropicalNights(5), "5 nuits tropicales");
assert.equal(formatTropicalNights(null), "non disponible");
...
assert.equal(climateCopy.description.includes("40 °C"), false, "fixture without daysGe40 must not invent extreme hot days");
assert.equal(climateCopy.description.includes("jours de pluie"), false, "fixture without daysRain must not invent rain days");
assert.equal(climateCopy.description.includes("gel"), false, "fixture without daysFrost must not invent frost days");
assert.equal(climateCopy.description.includes("tropicale"), false, "fixture without tropicalNights must not invent tropical nights");
...
assert.equal(climateNoPrecip.description.includes("jours de pluie"), false, "incomplete precip must not invent rain days");
...
assert.ok(homeSrc.includes("formatDaysGe25"), "home cards show official days at or above 25 °C");
assert.ok(homeSrc.includes("formatDaysGe30"), "home cards show official days at or above 30 °C");
...
assert.ok(explorerSrc.includes("Plus de nuits tropicales"), "year records show the observed tropical-night maximum of the series");
assert.ok(explorerSrc.includes("yearRecords.mostTropicalNights"));
assert.ok(explorerSrc.includes("Plus de jours ≥ 35 °C"), "year records show the observed very-hot-day maximum of the series");
...
assert.ok(explorerSrc.includes("climateLead.daysGe40"));
assert.ok(explorerSrc.includes("Jours de pluie"), "climate hero shows official rain days when precip is complete");
assert.ok(explorerSrc.includes("climateLead.daysRain"));
assert.ok(explorerSrc.includes("Jours de gel"), "climate hero shows official frost days");
assert.ok(explorerSrc.includes("daysFrost"));
assert.ok(explorerSrc.includes("Nuits tropicales"), "climate hero shows official tropical nights");
assert.ok(explorerSrc.includes("climateLead.tropicalNights"));
...
assert.ok(climateOgSrc.includes("Jours à 35 °C ou plus"));
assert.ok(climateOgSrc.includes("daysGe35"));
assert.ok(climateOgSrc.includes("Jours à 40 °C ou plus"));
assert.ok(climateOgSrc.includes("daysGe40"));
assert.ok(climateOgSrc.includes("Jours de pluie"));
assert.ok(climateOgSrc.includes("daysRain"));
assert.ok(climateOgSrc.includes("Jours de gel"));
assert.ok(climateOgSrc.includes("daysFrost"));
assert.ok(climateOgSrc.includes("Nuits tropicales"));
assert.ok(climateOgSrc.includes("tropicalNights"));
...
assert.ok(homeOgSrc.includes("Jours de pluie"));
assert.ok(homeOgSrc.includes("Nuits tropicales"));
...
const tropicalNightRecords = observedYearRecords([
  { year: 2026, tminMean: 9, tmaxMean: 22, precipitationSum: null, daysGe30: 1, tropicalNights: 99, yearComplete: false, precipComplete: false },
  { year: 2015, tminMean: 8, tmaxMean: 18, precipitationSum: 800, daysGe30: 5, tropicalNights: 5, yearComplete: true, precipComplete: true },
  { year: 2024, tminMean: 8, tmaxMean: 19, precipitationSum: 700, daysGe30: 8, tropicalNights: 7, yearComplete: true, precipComplete: true },
  { year: 2025, tminMean: 8, tmaxMean: 19, precipitationSum: 900, daysGe30: 10, tropicalNights: 5, yearComplete: true, precipComplete: true }
]);
assert.equal(tropicalNightRecords.mostTropicalNights?.year, 2024);
assert.equal(tropicalNightRecords.mostTropicalNights?.value, 7);
assert.notEqual(tropicalNightRecords.mostTropicalNights?.value, 99, "incomplete years must not set the tropical-night year record");
assert.notEqual(tropicalNightRecords.mostTropicalNights?.value, 5, "2025 tropical nights must not become the series maximum");
const tiedTropicalNights = observedYearRecords([
  { year: 2015, tminMean: 8, tmaxMean: 18, precipitationSum: 500, daysGe30: 5, tropicalNights: 7, yearComplete: true, precipComplete: true },
  { year: 2024, tminMean: 8, tmaxMean: 18, precipitationSum: 500, daysGe30: 5, tropicalNights: 7, yearComplete: true, precipComplete: true }
]);
assert.equal(tiedTropicalNights.mostTropicalNights?.year, 2015, "a tropical-night record tie keeps the earlier complete year");
assert.equal(tiedTropicalNights.mostTropicalNights?.value, 7);
const daysGe35Records = observedYearRecords([
  { year: 2026, tminMean: 9, tmaxMean: 22, precipitationSum: null, daysGe30: 1, daysGe35: 99, yearComplete: false, precipComplete: false },
  { year: 2003, tminMean: 8, tmaxMean: 22, precipitationSum: 400, daysGe30: 20, daysGe35: 22, yearComplete: true, precipComplete: true },
  { year: 2022, tminMean: 8, tmaxMean: 20, precipitationSum: 700, daysGe30: 18, daysGe35: 18, yearComplete: true, precipComplete: true },
  { year: 2025, tminMean: 8, tmaxMean: 19, precipitationSum: 900, daysGe30: 15, daysGe35: 15, yearComplete: true, precipComplete: true }
]);
assert.equal(daysGe35Records.mostDaysGe35?.year, 2003);
assert.equal(daysGe35Records.mostDaysGe35?.value, 22);
assert.notEqual(daysGe35Records.mostDaysGe35?.value, 99, "incomplete years must not set the 35 °C year record");
assert.notEqual(daysGe35Records.mostDaysGe35?.value, 18, "2022 35 °C days must not become the series maximum");
assert.notEqual(daysGe35Records.mostDaysGe35?.value, 15, "2025 35 °C days must not become the series maximum");
const tiedDaysGe35 = observedYearRecords([
  { year: 2003, tminMean: 8, tmaxMean: 18, precipitationSum: 500, daysGe30: 20, daysGe35: 22, yearComplete: true, precipComplete: true },
  { year: 2022, tminMean: 8, tmaxMean: 18, precipitationSum: 500, daysGe30: 20, daysGe35: 22, yearComplete: true, precipComplete: true }
]);
assert.equal(tiedDaysGe35.mostDaysGe35?.year, 2003, "a 35 °C record tie keeps the earlier complete year");
assert.equal(tiedDaysGe35.mostDaysGe35?.value, 22);
const daysGe25Records = observedYearRecords([
  { year: 2026, tminMean: 9, tmaxMean: 22, precipitationSum: null, daysGe30: 1, daysGe25: 199, yearComplete: false, precipComplete: false },
  { year: 2018, tminMean: 8, tmaxMean: 20, precipitationSum: 700, daysGe30: 12, daysGe25: 135, yearComplete: true, precipComplete: true },
  { year: 2022, tminMean: 8, tmaxMean: 20, precipitationSum: 700, daysGe30: 18, daysGe25: 126, yearComplete: true, precipComplete: true },
  { year: 2025, tminMean: 8, tmaxMean: 19, precipitationSum: 900, daysGe30: 15, daysGe25: 110, yearComplete: true, precipComplete: true }
]);
assert.equal(daysGe25Records.mostDaysGe25?.year, 2018);
assert.equal(daysGe25Records.mostDaysGe25?.value, 135);
assert.notEqual(daysGe25Records.mostDaysGe25?.value, 199, "incomplete years must not set the 25 °C year record");
assert.notEqual(daysGe25Records.mostDaysGe25?.value, 126, "2022 25 °C days must not become the series maximum");
assert.notEqual(daysGe25Records.mostDaysGe25?.value, 110, "2025 25 °C days must not become the series maximum");
const tiedDaysGe25 = observedYearRecords([
  { year: 2018, tminMean: 8, tmaxMean: 18, precipitationSum: 500, daysGe30: 12, daysGe25: 135, yearComplete: true, precipComplete: true },
  { year: 2022, tminMean: 8, tmaxMean: 18, precipitationSum: 500, daysGe30: 18, daysGe25: 135, yearComplete: true, precipComplete: true }
]);
...
assert.equal(tiedHotDays.mostDaysGe30?.year, 2001, "a hot-day record tie keeps the earlier complete year");
assert.equal(tiedHotDays.mostDaysGe30?.value, 20);
...
assert.equal(climateCard.daysRainDisplay?.includes("205"), false, "climate OG rain days are 2025, not the 2014 count");
assert.equal(climateCard.daysGe25Display, "110 j");
assert.equal(climateCard.daysGe25Display?.includes("135"), false, "climate OG 25 °C days are 2025, not the 2018 record");
assert.equal(climateCard.daysGe25Display?.includes("126"), false, "climate OG 25 °C days are 2025, not the 2022 count");
assert.equal(climateCard.daysGe30Display, "57 j");
assert.equal(climateCard.daysGe35Display, "15 j");
assert.equal(climateCard.daysGe35Display?.includes("18"), false, "climate OG 35 °C days are 2025, not the 2022 count");
assert.equal(climateCard.daysGe35Display?.includes("22"), false, "climate OG 35 °C days are 2025, not the 2003 record");
assert.equal(climateCard.daysGe40Display, "0 j");
assert.notEqual(climateCard.daysGe40Display, null, "a true zero of 40 °C days must be shown, not omitted");
assert.equal(climateCard.daysGe40Display?.includes("8"), false, "climate OG 40 °C days are LVD 2025, not another station");
assert.equal(climateCard.daysFrostDisplay, "46 j");
assert.equal(climateCard.tropicalNightsDisplay, "5 nuits");
assert.equal(climateCard.tropicalNightsDisplay?.includes("7"), false, "climate OG tropical nights are 2025, not the 2024 record");
assert.equal(climateCard.daysFrostDisplay?.includes("94"), false, "climate OG frost is 2025, not the 2005 record");
assert.equal(climateCard.daysGe30Display?.includes("71"), false, "climate OG hot days are 2025, not the 2022 record");
...
assert.ok(climateShare.includes("57 jours ≥ 30 °C"));
assert.ok(climateShare.includes("15 jours ≥ 35 °C"));
assert.ok(climateShare.includes("0 jour ≥ 40 °C"));
assert.ok(climateShare.includes("46 jours de gel"));
assert.ok(climateShare.includes("5 nuits tropicales"));
assert.equal(climateShare.includes("135 jours ≥ 25"), false, "share text must not paste the 2018 25 °C record");
assert.equal(climateShare.includes("126 jours ≥ 25"), false, "share text must not paste the 2022 25 °C count");
assert.equal(climateShare.includes("18 jours ≥ 35"), false, "share text must not paste the 2022 35 °C count");
assert.equal(climateShare.includes("22 jours ≥ 35"), false, "share text must not paste the 2003 35 °C record");
assert.equal(climateShare.includes("8 jours ≥ 40"), false, "share text must not paste another station's 40 °C count");
assert.equal(climateShare.includes("210 jours de pluie"), false, "share text must not paste the 2001 rain-day record");
assert.equal(climateShare.includes("205 jours de pluie"), false, "share text must not paste the 2014 rain-day count");
assert.equal(climateShare.includes("7 nuits"), false, "share text must not paste the 2024 tropical-night record");
assert.equal(climateShare.includes("94"), false, "share text must not paste the 2005 frost record");
assert.equal(climateShare.includes("71"), false, "share text must not paste the 2022 hot-day record");
...
assert.equal(climateShareNoMin.includes("30 °C"), false, "omitted hot-day count must not be invented in the share text");
assert.equal(climateShareNoMin.includes("35 °C"), false, "omitted 35 °C count must not be invented in the share text");
assert.equal(climateShareNoMin.includes("40 °C"), false, "omitted 40 °C count must not be invented in the share text");
assert.equal(climateShareNoMin.includes("gel"), false, "omitted frost count must not be invented in the share text");
assert.equal(climateShareNoMin.includes("tropicale"), false, "omitted tropical-night count must not be invented in the share text");
...
assert.equal(climateCardNoRain?.daysGe35Display, null, "omitted 35 °C count must not be invented on the OG card");
assert.equal(climateCardNoRain?.daysGe40Display, null, "omitted 40 °C count must not be invented on the OG card");
assert.equal(climateCardNoRain?.daysFrostDisplay, null, "omitted frost count must not be invented on the OG card");
assert.equal(climateCardNoRain?.tropicalNightsDisplay, null, "omitted tropical-night count must not be invented on the OG card");
...
if (obsCount === 0) {
  ...
  assert.equal(yearly.yearRecords.mostDaysGe30?.year, 2003, "LVD hot-day year record is 2003, not 2022");
  assert.equal(yearly.yearRecords.mostDaysGe30?.value, 73);
  assert.notEqual(yearly.yearRecords.mostDaysGe30?.value, 71, "2022 has 71 days ≥ 30 °C, not the series maximum");
  assert.notEqual(yearly.yearRecords.mostDaysGe30?.value, 57, "year records must not paste 2025 onto the hot-day maximum");
  ...
  assert.equal(yearly.yearRecords.mostTropicalNights?.year, 2024, "LVD tropical-night year record is 2024, not 2025");
  assert.equal(yearly.yearRecords.mostTropicalNights?.value, 7);
  assert.notEqual(yearly.yearRecords.mostTropicalNights?.value, 5, "year records must not paste 2025 onto the tropical-night maximum");
  const mostTropYear = yearly.years.find((row) => row.year === yearly.yearRecords.mostTropicalNights?.year);
  assert.equal(mostTropYear?.yearComplete, true);
  assert.equal(mostTropYear?.tropicalNights, 7);
  assert.equal(yearly.yearRecords.mostDaysGe35?.year, 2003, "LVD 35 °C year record is 2003, not 2022");
  assert.equal(yearly.yearRecords.mostDaysGe35?.value, 22);
  assert.notEqual(yearly.yearRecords.mostDaysGe35?.value, 18, "2022 has 18 days ≥ 35 °C, not the series maximum");
  assert.notEqual(yearly.yearRecords.mostDaysGe35?.value, 15, "year records must not paste 2025 onto the 35 °C maximum");
  const mostGe35Year = yearly.years.find((row) => row.year === yearly.yearRecords.mostDaysGe35?.year);
  assert.equal(mostGe35Year?.yearComplete, true);
  assert.equal(mostGe35Year?.daysGe35, 22);
  assert.equal(yearly.yearRecords.mostDaysGe25?.year, 2018, "LVD 25 °C year record is 2018, not 2022");
  assert.equal(yearly.yearRecords.mostDaysGe25?.value, 135);
  assert.notEqual(yearly.yearRecords.mostDaysGe25?.value, 126, "2022 has 126 days ≥ 25 °C, not the series maximum");
  assert.notEqual(yearly.yearRecords.mostDaysGe25?.value, 110, "year records must not paste 2025 onto the 25 °C maximum");
  ...
  assert.notEqual(grenobleHome.lastComplete?.daysRain, 205, "home cards must not paste the 2014 rain-day count onto 2025");
  assert.notEqual(grenobleHome.lastComplete?.daysRain, 89, "home cards must not paste incomplete 2026 rain days onto 2025");
  assert.equal(grenobleHome.lastComplete?.daysGe25, 110);
  assert.equal(grenobleHome.lastComplete?.daysGe25, yearly2025.daysGe25);
  assert.notEqual(grenobleHome.lastComplete?.daysGe25, 135, "home cards must not paste the 2018 25 °C record onto 2025");
  assert.notEqual(grenobleHome.lastComplete?.daysGe25, 126, "home cards must not paste the 2022 25 °C count onto 2025");
  assert.equal(grenobleHome.lastComplete?.daysGe30, 57);
  assert.equal(grenobleHome.lastComplete?.daysGe30, yearly2025.daysGe30);
  assert.notEqual(grenobleHome.lastComplete?.daysGe30, 71, "home cards must not paste the 2022 hot-day count onto 2025");
  assert.notEqual(grenobleHome.lastComplete?.daysGe30, 73, "home cards must not paste the 2003 hot-day record onto 2025");
  assert.equal(grenobleHome.lastComplete?.daysGe35, 15);
  assert.equal(grenobleHome.lastComplete?.daysGe35, yearly2025.daysGe35);
  assert.notEqual(grenobleHome.lastComplete?.daysGe35, 18, "home cards must not paste the 2022 35 °C count onto 2025");
  assert.notEqual(grenobleHome.lastComplete?.daysGe35, 22, "home cards must not paste the 2003 35 °C record onto 2025");
  assert.equal(grenobleHome.lastComplete?.daysGe40, 0);
  assert.equal(grenobleHome.lastComplete?.daysGe40, yearly2025.daysGe40);
  assert.notEqual(grenobleHome.lastComplete?.daysGe40, null, "home cards must show a true zero of 40 °C days, not hide it");
  assert.notEqual(grenobleHome.lastComplete?.daysGe40, 8, "home cards must not paste another station's 40 °C count onto LVD");
  ...
  assert.equal(crollesHome.lastComplete?.daysGe30, grenobleHome.lastComplete?.daysGe30);
  assert.equal(crollesHome.lastComplete?.daysGe35, grenobleHome.lastComplete?.daysGe35);
  assert.equal(crollesHome.lastComplete?.daysGe40, grenobleHome.lastComplete?.daysGe40);
  assert.equal(crollesHome.lastComplete?.daysFrost, grenobleHome.lastComplete?.daysFrost);
  assert.equal(crollesHome.lastComplete?.tropicalNights, grenobleHome.lastComplete?.tropicalNights);
  ...
  assert.notEqual(grenobleClimate?.daysRain, 205, "canonical climate is 2025 rain days, not the 2014 count");
  assert.notEqual(grenobleClimate?.daysRain, 89, "canonical climate is 2025 rain days, not incomplete 2026");
  assert.equal(grenobleClimate?.daysGe25, 110);
  assert.notEqual(grenobleClimate?.daysGe25, 135, "canonical climate is 2025 25 °C days, not the 2018 record");
  assert.notEqual(grenobleClimate?.daysGe25, 126, "canonical climate is 2025 25 °C days, not the 2022 count");
  assert.equal(grenobleClimate?.daysGe30, 57);
  assert.notEqual(grenobleClimate?.daysGe30, 73, "canonical climate is 2025 hot days, not the 2003 record");
  assert.notEqual(grenobleClimate?.daysGe30, 71, "canonical climate is 2025 hot days, not the 2022 count");
  assert.equal(grenobleClimate?.daysGe35, 15);
  assert.notEqual(grenobleClimate?.daysGe35, 18, "canonical climate is 2025 35 °C days, not the 2022 count");
  assert.notEqual(grenobleClimate?.daysGe35, 22, "canonical climate is 2025 35 °C days, not the 2003 record");
  assert.equal(grenobleClimate?.daysGe40, 0);
  assert.notEqual(grenobleClimate?.daysGe40, null, "canonical climate must show a true zero of 40 °C days");
  ...
  if (incomplete1999 && !incomplete1999.precipComplete) {
    assert.equal(incomplete1999.daysRain, undefined, "incomplete 1999 rain-day count must not be shown");
    assert.notEqual(incomplete1999.daysRain, 131);
  }
  ...
  assert.equal(homeOg.daysRainDisplay?.includes("205"), false);
  assert.equal(homeOg.daysGe25Display, "110 j");
  assert.equal(homeOg.daysGe25Display?.includes("135"), false);
  assert.equal(homeOg.daysGe25Display?.includes("126"), false);
  assert.equal(homeOg.daysGe30Display, "57 j");
  assert.equal(homeOg.daysGe35Display, "15 j");
  assert.equal(homeOg.daysGe35Display?.includes("18"), false);
  assert.equal(homeOg.daysGe35Display?.includes("22"), false);
  assert.equal(homeOg.daysGe40Display, "0 j");
  assert.notEqual(homeOg.daysGe40Display, null, "home OG must show a true zero of 40 °C days");
  assert.equal(homeOg.daysGe40Display?.includes("8"), false);
  assert.equal(homeOg.daysFrostDisplay, "46 j");
  assert.equal(homeOg.tropicalNightsDisplay, "5 nuits");
  assert.equal(homeOg.tropicalNightsDisplay?.includes("7"), false);
  assert.equal(homeOg.daysFrostDisplay?.includes("94"), false);
  assert.equal(homeOg.daysGe30Display?.includes("71"), false);
  ...
  assert.ok(grenobleClimateCopy.description.includes("57 jours ≥ 30 °C"));
  assert.ok(grenobleClimateCopy.description.includes("15 jours ≥ 35 °C"));
  assert.ok(grenobleClimateCopy.description.includes("0 jour ≥ 40 °C"));
  assert.ok(grenobleClimateCopy.description.includes("46 jours de gel"));
  assert.ok(grenobleClimateCopy.description.includes("5 nuits tropicales"));
  assert.equal(grenobleClimateCopy.description.includes("135 jours ≥ 25"), false, "canonical description is not the 2018 25 °C record");
  assert.equal(grenobleClimateCopy.description.includes("126 jours ≥ 25"), false, "canonical description is not the 2022 25 °C count");
  assert.equal(grenobleClimateCopy.description.includes("71"), false, "canonical description is not the 2022 hot-day record");
  assert.equal(grenobleClimateCopy.description.includes("94"), false, "canonical description is not the 2005 frost record");
  assert.equal(grenobleClimateCopy.description.includes("57 jours de gel"), false, "2022 frost must not replace 2025");
  assert.equal(grenobleClimateCopy.description.includes("7 nuits"), false, "canonical description is not the 2024 tropical-night record");
  assert.equal(grenobleClimateCopy.description.includes("18 jours ≥ 35"), false, "canonical description is not the 2022 35 °C count");
  assert.equal(grenobleClimateCopy.description.includes("22 jours ≥ 35"), false, "canonical description is not the 2003 35 °C record");
  assert.equal(grenobleClimateCopy.description.includes("8 jours ≥ 40"), false, "canonical description is not another station's 40 °C count");
  assert.equal(grenobleClimateCopy.description.includes("210 jours de pluie"), false, "canonical description is not the 2001 rain-day record");
  assert.equal(grenobleClimateCopy.description.includes("205 jours de pluie"), false, "canonical description is not the 2014 rain-day count");
  ...
  assert.equal(crollesClimate?.daysGe30, grenobleClimate?.daysGe30);
  assert.equal(crollesClimate?.daysGe35, grenobleClimate?.daysGe35);
  assert.equal(crollesClimate?.daysGe40, grenobleClimate?.daysGe40);
  assert.equal(crollesClimate?.daysFrost, grenobleClimate?.daysFrost);
  assert.equal(crollesClimate?.tropicalNights, grenobleClimate?.tropicalNights);
